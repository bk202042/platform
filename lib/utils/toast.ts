...
export const toastUtils = {
  ...
  // Error messages
  error: {
    generic: (message?: string) => toast.error('오류가 발생했습니다', {
      description: message || '다시 시도해주세요.'
    }),
    network: () => toast.error('네트워크 오류', {
    ...
    validation: (message: string) => toast.error('입력 오류', {
      description: message
    }),
    server: () => toast.error('서버 오류', {
      description: '일시적인 서버 문제입니다. 잠시 후 다시 시도해주세요.'
    }),
    notFound: (resource: string = '데이터') => toast.error('찾을 수 없음', {
      description: `요청하신 ${resource}를 찾을 수 없습니다.`
    }),
    permission: () => toast.error('권한 없음', {
      description: '이 작업을 수행할 권한이 없습니다.'
    }),
    imageUpload: (message?: string) => toast.error('이미지 업로드 실패', {
      description: message || '이미지 업로드 중 오류가 발생했습니다.'
    }),
    fileTooLarge: (maxSize: string) => toast.error('파일 크기 초과', {
      description: `파일 크기는 ${maxSize} 이하여야 합니다.`
    }),
    invalidFileType: () => toast.error('지원하지 않는 파일 형식', {
  ...
  // Warning messages
  warning: {
    unsavedChanges: () => toast.warning('저장되지 않은 변경사항', {
      description: '변경사항이 저장되지 않았습니다.'
    }),
    sessionExpiring: () => toast.warning('세션 만료 예정', {
    ...
    slowConnection: () => toast.warning('연결 속도가 느립니다', {
  ...
  info: {
    ...
    offline: () => toast.info('오프라인 모드', {
      description: '인터넷 연결이 복구되면 자동으로 동기화됩니다.'
    }),
    maintenance: () => toast.info('시스템 점검', {
